...
/* ---- Streamlit container (cards rendered via st.container(border=True)) -- */
...
/* ---- Hide Streamlit's native top header bar (black strip in screenshot) -- */
...
/* ---- Number input stepper buttons (kill the black lat/lon backgrounds) --- */
...
/* ---- Brand tagline wrapping on narrow viewports ------------------- */
...
/* ---- KPI text colors readable on white --------------------------- */
.kpi-label, .sec-label { color: #475569 !important; }
...
/* ---- Captions a touch darker for readability ---------------------- */
[data-testid="stCaptionContainer"], .stCaption {
    color: #6B7280 !important;
    font-size: 0.82rem !important;
}

/* ---- Inline kpi-card content (used inside El Nino tab) ------------ */
.kpi-card-inline {
    border: 1px solid #E5E7EB;
    border-radius: 6px;
    padding: 16px 20px;
    ...
    if is_past_mode:
        ...
        with st.container(border=True):
            ...
            if not hs.get("ok"):
                ...
                with cA:
                    st.markdown(
                        f"<div class='card' style='border-color:#E5E7EB'>"
                ...
                with cC:
                    if hs["enso_conditional_precip"] is not None:
                        ...
                        st.markdown(
                            f"<div class='card' style='border-color:{ph_color}99'>"
                        ...
                        st.markdown(
                            f"<div class='card' style='opacity:.55;border-color:#E5E7EB'>"
